...
<div class="max-w-3xl mx-auto flex flex-col gap-6">
    <div class="flex items-center gap-2">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-[#1b1b18] dark:text-[#EDEDEC]">Edit Job</h1>
    </div>
    ...
    <div class="bg-white dark:bg-[#161615] p-6 rounded-lg border border-[#19140035] dark:border-[#3E3E3A] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.06)]">
        <form action="{{ route('jobs.update', $job) }}" method="POST" class="flex flex-col gap-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="flex flex-col gap-2">
                    <label for="name" class="text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">Job Title <span class="text-[#f53003]">*</span></label>
                    <input type="text" id="name" name="name" value="{{ old('name', $job->name) }}" placeholder="e.g. Lead PHP Developer"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="flex flex-col gap-2">
                    ...
                    <select id="plan_duration" name="plan_duration" required
                        ...
                        @foreach (['1 Month','2 Months','3 Months','6 Months','1 Year'] as $dur)
                            <option value="{{ $dur }}" {{ old('plan_duration', $job->plan_duration) === $dur ? 'selected' : '' }}>{{ $dur }}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="date" id="end_date" name="end_date"
                        value="{{ old('end_date', $job->end_date?->format('Y-m-d')) }}" readonly
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-[#19140035] dark:border-[#3E3E3A]">
                ...
                <button type="submit" class="px-4 py-2 text-sm font-medium rounded-md text-white bg-[#f53003] dark:bg-[#FF4433] hover:bg-black dark:hover:bg-white dark:hover:text-[#1C1C1A] transition-colors shadow-sm">Update Job</button>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const currentState = "{{ old('state', $job->state) }}";
    ...
    fetch("{{ route('api.indian-states') }}")
        ...
        .then(states => {
            ...
            if (currentState) loadCities(currentState, currentCity);
        })
