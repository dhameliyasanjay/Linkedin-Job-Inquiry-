...
--}}
@forelse ($jobs as $job)
    @php $expired = $job->isExpired(); @endphp
    <tr class="{{ $expired ? 'bg-red-50 dark:bg-red-950/20' : 'hover:bg-gray-50/50 dark:hover:bg-[#1b1b18]/30' }} transition-colors">
        ...
        <td class="px-5 py-4 text-[#706f6c] dark:text-[#A1A09A]">{{ $job->city ?: '—' }}</td>
        <td class="px-5 py-4 text-[#706f6c] dark:text-[#A1A09A]">{{ $job->experience ?? '—' }}</td>
        ...
        <td class="px-5 py-4 text-right">
            <div class="flex justify-end gap-3">
                <a href="{{ route('jobs.show', $job) }}" class="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 hover:text-emerald-800 dark:text-emerald-400 transition-colors">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    View
                </a>
                <a href="{{ route('jobs.edit', $job) }}" class="inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 transition-colors">
                ...
                <form action="{{ route('jobs.destroy', $job) }}" method="POST" class="inline delete-job-form">
                    @csrf
                    @method('DELETE')
                    <button type="button" class="inline-flex items-center gap-1 text-xs font-medium text-[#f53003] hover:text-[#f53003]/80 dark:text-[#FF4433] transition-colors delete-btn">
...
@empty
    <tr>
        <td colspan="9" class="px-6 py-12 text-center text-sm text-[#706f6c] dark:text-[#A1A09A]">
